...
CANDIDATE_TABLE: dict[FailureCauseId, tuple[RepairCandidate, ...]] = {
    ...
    FailureCauseId.OOD_TOO_HARD: (
        RepairCandidate(
            id="OOD_TOO_HARD_severity_down_mass",
            cause_id=FailureCauseId.OOD_TOO_HARD,
            patch={"ood_mass": 1.3},
            cost_minutes=20,
            risk=0.2,
            expected_signal=0.5,
            description="Reduce mass OOD severity (1.5 -> 1.3).",
            applicable_phases=("R2",),
        ),
        RepairCandidate(
            id="OOD_TOO_HARD_severity_down_friction",
            cause_id=FailureCauseId.OOD_TOO_HARD,
            patch={"ood_friction": 3.0},
            cost_minutes=20,
            risk=0.2,
            expected_signal=0.5,
            description="Reduce friction OOD severity (5.0 -> 3.0).",
            applicable_phases=("R2",),
        ),
        RepairCandidate(
            id="OOD_TOO_HARD_expand_coverage",
            cause_id=FailureCauseId.OOD_TOO_HARD,
            patch={"num_episodes": 200},
            cost_minutes=40,
            risk=0.1,
            expected_signal=0.3,
            description="Double episode count to improve OOD coverage.",
            applicable_phases=("R2",),
        ),
    ),
    FailureCauseId.EVAL_NOISE_HIGH: (
        RepairCandidate(
            id="EVAL_NOISE_HIGH_more_seeds",
            cause_id=FailureCauseId.EVAL_NOISE_HIGH,
            patch={"n_eval_seeds": 5},
            cost_minutes=15,
            risk=0.1,
            expected_signal=0.5,
            description="Increase eval seed count to 5 for better CI.",
            applicable_phases=("R3", "R4", "R5", "R7", "R9", "R10"),
        ),
        RepairCandidate(
            id="EVAL_NOISE_HIGH_longer_episode",
            cause_id=FailureCauseId.EVAL_NOISE_HIGH,
            patch={"max_episode_steps": 200},
            cost_minutes=30,
            risk=0.2,
            expected_signal=0.4,
            description="Extend episode length for lower eval variance.",
            applicable_phases=("R3", "R4", "R5", "R7"),
        ),
    ),
